...
class All_Category_Combinations
{
    ...
    // HASH //
    void create_hash_table_for_this_set_of_params(const string& hash_table_name, const vector<string>& X_line, const vector<vector<string>>& LINE_combinations, const vector<vector<string>>& GROUP_CHART_combinations)
    {
        hash_map.clear();
...
int main(int argc, char* argv[])
{
    ...
    #define PACKED
        ...
        // WYKRESY PER THREAD //
        comb.start(NUM_OF_THREADS, {SCENE, UNIT, ARCH}, {MODEL}, true, {LIGHTS, SPHERES, BOUNCES, PHYSICAL_CORES, LOGICAL_CORES, CORE_PROPORTION, RATING});

        // Z POMIJANIEM //
        comb.start(SCENE, {UNIT}, {RATING}, true, {LIGHTS, SPHERES, BOUNCES, NUM_OF_THREADS, PHYSICAL_CORES, LOGICAL_CORES, CORE_PROPORTION});

        // każdy model po koleji
        comb.start(MODEL, {SCENE}, {PROCESSOR, UNIT, RATING}, true, {LIGHTS, SPHERES, BOUNCES, NUM_OF_THREADS, PHYSICAL_CORES, LOGICAL_CORES, CORE_PROPORTION});
        comb.start(MODEL, {UNIT}, {SCENE, RATING}, true, {LIGHTS, SPHERES, BOUNCES, NUM_OF_THREADS, PHYSICAL_CORES, LOGICAL_CORES, CORE_PROPORTION});

        // per phisical core -> z pośród tych co mają tyle rdzeni, który działa najlepiej
        comb.start(PHYSICAL_CORES, {SCENE, MODEL, ARCH}, {UNIT, RATING}, true, {LIGHTS, SPHERES, BOUNCES, NUM_OF_THREADS, LOGICAL_CORES, CORE_PROPORTION});
    #endif
