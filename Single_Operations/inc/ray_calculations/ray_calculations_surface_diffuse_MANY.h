...
#ifdef GPU
__global__ void ray_calculations_surface_diffuse_MANY_Kernel(u64 ARRAY_SIZE, Ray_calculations::d3* dev_points_on_a_plane, Ray_calculations::Ray* dev_normal, Ray_calculations::Ray* dev_output_rays, unsigned long seed)
{
    ...
    for(u64 point_index=0; point_index < ARRAY_SIZE; point_index++)
    {
        #ifdef UNIT_FLOAT
        ...
        REAL_TASK(r_x, r_y, r_z, dev_points_on_a_plane[point_index], (*dev_normal));
    }
}
#endif

class ray_calculations_surface_diffuse_MANY : public ray_calculations_BASE
{
    #ifdef CPU
        void cpu_task(int i)
        {
            for(u64 point_index=0; point_index < ARRAY_SIZE; point_index++)
            {
                ...
                REAL_TASK(r_x, r_y, r_z, host_points_on_a_plane[point_index], host_normal);
            }
        }
    #endif
        ...
        void cuda_launch()
        {
            ray_calculations_surface_diffuse_MANY_Kernel<<<NUMBER_OF_BLOCKS, BLOCK_SIZE>>>(ARRAY_SIZE, dev_points_on_a_plane, dev_normal, dev_output_rays, time(0));
...
public:
    ray_calculations_surface_diffuse_MANY(string name, u64 s)
