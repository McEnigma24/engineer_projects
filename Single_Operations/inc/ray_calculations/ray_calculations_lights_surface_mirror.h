...
#ifdef GPU
__global__ void ray_calculations_lights_surface_mirror_Kernel(Ray_calculations::d3* dev_points_on_a_plane, Ray_calculations::Ray* dev_normal, Light_point* dev_light_points, Ray_calculations::Ray* dev_output_rays)
{
    int i = threadIdx.x + blockIdx.x * blockDim.x;

    REAL_TASK(dev_output_rays[i], dev_points_on_a_plane[i], (*dev_normal), dev_light_points[0]);
}
#endif

class ray_calculations_lights_surface_mirror : public ray_calculations_lights_BASE
{
    #ifdef CPU
        void cpu_task(int i)
        {
            REAL_TASK(host_output_rays[i], host_points_on_a_plane[i], host_normal, host_light_points[0]);
        }
    #endif
        ...
        void cuda_launch()
        {
            ray_calculations_lights_surface_mirror_Kernel<<<NUMBER_OF_BLOCKS, BLOCK_SIZE>>>(dev_points_on_a_plane, dev_normal, dev_light_points, dev_output_rays);
...
public:
    ray_calculations_lights_surface_mirror(string name, u64 s)
