...
unit intensity_based_on_angle_MANY(const Ray_calculations::Ray& light_ray, const Ray_calculations::Ray& normal)
{
...
{ \
 ...
 \
        unit intensity = intensity_based_on_angle_MANY(ray_from_point_to_light_point, *normal); \
...
#ifdef GPU
__global__ void ray_calculations_lights_intensity_based_on_angle_MANY_Kernel(u64 ARRAY_SIZE, Ray_calculations::d3* dev_points_on_a_plane, Ray_calculations::Ray* dev_normal, Light_point* dev_light_points, RGB_float* dev_output_color)
{
...
class ray_calculations_lights_intensity_based_on_angle_MANY : public ray_calculations_lights_BASE
{
    #ifdef CPU
        void cpu_task(int i)
        {
        ...
        void cuda_launch()
        {
            ray_calculations_lights_intensity_based_on_angle_MANY_Kernel<<<NUMBER_OF_BLOCKS, BLOCK_SIZE>>>(ARRAY_SIZE, dev_points_on_a_plane, dev_normal, dev_light_points, dev_output_color);
...
public:
    ray_calculations_lights_intensity_based_on_angle_MANY(string name, u64 s)
